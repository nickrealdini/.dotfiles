...
beautiful.init("/home/ktr/.config/awesome/ktheme.lua")
...
-- {{{ Wibox
-- CLOCK
mytextclock = awful.widget.textclock("<span color='#FFFFFF'> %a %d/%m | <b>%H:%M</b></span> ",  2)
-- mytextclock = awful.widget.textclock(" %a %d/%m | <b>%H:%M</b> ",  2)
...
for s = 1, screen.count() do
    ...
    function batteryInfo(adapter)
        ...
        local cap = fcap:read()
        local sta = fsta:read()
        -- fix because after suspend/wakeup there is an error
        if cap == nil then
            cap = 5243000
        end
        ...
        elseif sta:match("Discharging") then
            -- display warning when battery is less than 15%
            if tonumber(battery) < 15 then
                naughty.notify({ title      = "Battery Warning"
                    , text       = "Plug da battery bro !"..spacer..battery.."%"..spacer.."left !"
                    , timeout    = 10
                    , position   = "top_right"
...
globalkeys = awful.util.table.join(
  ...
  --  awful.key({ modkey, "Shift"   }, "t", function () awful.client.swap.byidx(  1)    end),
    ...
    awful.key({ modkey, "Control" }, "ç",     function () awful.tag.incncol( 1)         end),
